...
import {AuthObjectMapper} from "../../service/AuthObjectMapper";
...
export class AuthCreateResultParserComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const clientDataJSON = AuthObjectMapper.base64ToJSON(this.response.response.clientDataJSON);

    const attestationObject = this.response.response.attestationObject;
